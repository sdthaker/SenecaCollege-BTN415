...
int main() {
    ...
    if (bind(ServerSocket, (struct sockaddr*)&SvrAddr,
        ...
        std::cout << "Could not bind TCP socket to port" << std::endl;
    ...
    if (bind(ClientSocket, (struct sockaddr*)&Svr,
        sizeof(Svr)) == SOCKET_ERROR) {
        std::cout << "Could not bind TCP socket to address" << std::endl;
    ...
    do {
        ...
        /*char UDPBuffer[128] = {};
        recv(ClientSocket, RxBuffer, sizeof(UDPBuffer), 0);
        std::cout << "Response: " << UDPBuffer << std::endl;*/
