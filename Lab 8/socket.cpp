...
data_packet create_data_packet() {
    ...
    std::cout << "Enter packet name: ";
    std::cin >> my_packet.name;
    std::cout << "Enter id: ";
    std::cin >> my_packet.id;
    std::cout << "Enter size of packet: ";
    std::cin >> my_packet.size;

    my_packet.numbers = new int[my_packet.size];
    for (size_t i = 0; i < my_packet.size; i++) {
        std::cout << "Enter element " << i << ": ";
        std::cin >> my_packet.numbers[i];
    }
...
serialized_packet data_packet_serializer(data_packet packet) {
    char* my_serialized_packet = new char[16 * sizeof(char) + 1 * sizeof(int) +
        1 * sizeof(int) + 1 * sizeof(int) + packet.size * sizeof(int)];
    char* auxptr = my_serialized_packet;

    memcpy(auxptr, packet.name, 16 * sizeof(char));
    auxptr += 16 * sizeof(char);
    memcpy(auxptr, &packet.id, 1 * sizeof(int));
    auxptr += 1 * sizeof(int);
    memcpy(auxptr, &packet.size, 1 * sizeof(int));
    auxptr += 1 * sizeof(int);
    memcpy(auxptr, &packet.tail, 1 * sizeof(int));
    auxptr += 1 * sizeof(int);
    memcpy(auxptr, packet.numbers, packet.size * sizeof(int));

    serialized_packet output;
    output.data = my_serialized_packet;
    output.length = 16 * sizeof(char) + 
                    1 * sizeof(int) + 
                    1 * sizeof(int) + 
                    1 * sizeof(int) + 
...
data_packet data_packet_deserializer(char* serialized_packet) {
    ...
    auxptr += 16 * sizeof(char);
    memcpy(&deserialized_packet.id, auxptr, sizeof(int));
    auxptr += 1 * sizeof(int);
    memcpy(&deserialized_packet.size, auxptr, sizeof(int));
    auxptr += 1 * sizeof(int);
    memcpy(&deserialized_packet.tail, auxptr, sizeof(int));
    auxptr += 1 * sizeof(int);
    deserialized_packet.numbers = new int[deserialized_packet.size];
    memcpy(deserialized_packet.numbers, auxptr, sizeof(int) * deserialized_packet.size);
